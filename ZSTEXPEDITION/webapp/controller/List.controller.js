sap.ui.define([
    ...
    return BaseController.extend("com.telefonica.fiori.st.zexpedicion.controller.List", {
        ...
        onInit : function () {
            ...
            this._oList = oList; // nos guardamos el elemento "Lista" en el contexto a nivel global
            this._oList.setBusy(true); // dejamos los 3 puntitos esperando de fondo (detalle)
            this._oListTemplate = oList.getBindingInfo("items").template; // guardamos template de la lista
            oList.unbindAggregation("items"); // le desasignamos el servicio de expedicionSet
            this.onShowExpeditionInput();
            this.getRouter().navTo("list"); // siempre volvéte a la visualizacion de la LISTA (detalle)
        ...
        onSearchExp: function () {

            var sExpedition = this.byId('id-expedicion').getValue() // obtenemos el valor del input
            this._oList.setBusy(false); // sacamos los 3 puntitos 

            this._oList.bindAggregation("items",{
				path: "/expedicionSet",
				template: this._oListTemplate,
				filters: new Filter("idOperacion", FilterOperator.EQ, sExpedition)
			});


			this._pExpeditionDialog.then(function (oFragment) {
				oFragment.close();
			}.bind(this));
		},

        onShowExpeditionPopUp: function() {
            this._onMasterMatched(); // vuelve a visualizar solo la lista (detalle) 
            this.onShowExpeditionInput();
        },

        onShowExpeditionInput: function () {
            
			if (!this._pExpeditionDialog) {
				this._pExpeditionDialog = sap.ui.core.Fragment.load({
					id: this.getView().getId(),
					name: "com.telefonica.fiori.st.zexpedicion.view.fragments.Expedition",
					controller: this
        ...
        onSearch: function (oEvent) {
            ...
            if (sQuery) {
                this._oListFilterState.aSearch = [new Filter("idOperacion", FilterOperator.Contains, sQuery)];
            } else {
        ...
        onOpenViewSettings: function (oEvent) {
            ...
            if (!this.byId("viewSettingsDialog")) {
                Fragment.load({
                    id: this.getView().getId(),
                    name: "com.telefonica.fiori.st.zexpedicion.view.ViewSettingsDialog",
                    controller: this
        ...
        _showDetail: function (oItem) {
            ...
            this.getRouter().navTo("object", {
                objectId : oItem.getBindingContext().getProperty("idOperacion")
            }, bReplace);
        },
